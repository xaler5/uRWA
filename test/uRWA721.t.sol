...
contract uRWA721Test is Test {
    ...
    function test_Revert_Transfer_FromNotWhitelisted() public {
        ...
        vm.prank(user1);
        vm.expectRevert(abi.encodeWithSelector(IuRWA.TransferNotAllowed.selector, user1, user2, TOKEN_ID_1));
        token.transferFrom(user1, user2, TOKEN_ID_1);
    }

    function test_Revert_Transfer_ToNotWhitelisted() public {
        vm.prank(user1);
        vm.expectRevert(abi.encodeWithSelector(IuRWA.TransferNotAllowed.selector, user1, nonWhitelistedUser, TOKEN_ID_1));
    ...
    function test_Recall_Success_WhitelistedToWhitelisted() public {
        ...
        emit IuRWA.Recalled(user1, user2, TOKEN_ID_1);
        token.recall(user1, user2, TOKEN_ID_1);
        assertEq(token.ownerOf(TOKEN_ID_1), user2);
    }
    ...
    function test_Recall_Success_FromNonWhitelistedToWhitelisted() public {
        ...
        vm.expectEmit(true, true, true, true); // Recalled event
        emit IuRWA.Recalled(user1, user2, TOKEN_ID_1);
        token.recall(user1, user2, TOKEN_ID_1); // Succeeds as 'from' whitelist status is not checked
        assertEq(token.ownerOf(TOKEN_ID_1), user2);
    }
    ...
    function test_Recall_Success_WhitelistedToNonWhitelisted() public {
        ...
        vm.expectEmit(true, true, true, true); // Recalled event
        emit IuRWA.Recalled(user1, nonWhitelistedUser, TOKEN_ID_1);
        token.recall(user1, nonWhitelistedUser, TOKEN_ID_1); // Succeeds as 'to' whitelist status is not checked by recall
